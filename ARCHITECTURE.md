...
Customer onboarding is a derived state machine, not a manually checked progress flag: Prepare → Authorize → Choose destinations → Verify. The provider callback returns directly to destination selection. Readiness is derived from the platform gate, connection health, real selected accounts, provider-specific identities/capabilities, and messaging sender/compliance state. The live health endpoint revalidates encrypted credentials against the provider, confirms selected destinations are still visible, updates degraded state, and emits an audit event without creating or activating provider resources.

SendGrid and Twilio use the same setup state machine. SendGrid verifies its domain, exact sender, unsubscribe group, and signed Event Webhook. Twilio verifies account/service ownership, explicitly configures the signed inbound STOP webhook when authorized, and applies US A2P readiness only to US-default setup; campaign preflight still blocks any later US recipient unless A2P is verified.
