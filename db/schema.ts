...
export const workspaces = sqliteTable("workspaces", {
  ...
  approvalMode: integer("approval_mode", { mode: "boolean" }).notNull().default(true),
});
...
}, (table) => [uniqueIndex("idx_membership_workspace_user").on(table.workspaceId, table.userId)]);
...
}, (table) => [uniqueIndex("idx_integration_definition_slug").on(table.slug), index("idx_integration_definition_category").on(table.category)]);
...
}, (table) => [index("idx_connections_workspace_state").on(table.workspaceId, table.state)]);
...
}, (table) => [index("idx_campaigns_workspace_state").on(table.workspaceId, table.state)]);
...
export const contentItems = sqliteTable("content_items", {
  ...
  updatedAt: text("updated_at").notNull(),
}, (table) => [index("idx_content_campaign_state").on(table.campaignId, table.state), index("idx_content_schedule").on(table.scheduledAt)]);
...
export const contentVersions = sqliteTable("content_versions", {
  ...
  createdAt: text("created_at").notNull(),
}, (table) => [uniqueIndex("idx_content_version_unique").on(table.contentId, table.version)]);
...
}, (table) => [index("idx_approvals_state_created").on(table.state, table.createdAt)]);
...
}, (table) => [index("idx_sync_runs_sync_started").on(table.syncId, table.startedAt)]);

export const paidAdCampaigns = sqliteTable("paid_ad_campaigns", {
  id: text("id").primaryKey(),
  workspaceId: text("workspace_id").notNull(),
  name: text("name").notNull(),
...
}, (table) => [index("idx_paid_ads_workspace_state").on(table.workspaceId, table.state)]);
...
}, (table) => [index("idx_metrics_workspace_date").on(table.workspaceId, table.date)]);
