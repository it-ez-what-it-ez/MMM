...
function buildTemplate(input: TemplateInput): TemplateManifest {
  const assets = input.recommendedChannels.map((channel, index) => ({
    id: `${input.id}-${channel}`,
    channel,
    format: channel === "google_search" ? "responsive_search" as const : channel === "google_display" ? "responsive_display" as const : channel === "email" ? "email_html" as const : channel === "sms" ? "sms_text" as const : channel === "linkedin" ? "static_image" as const : index % 3 === 1 ? "carousel" as const : "static_image" as const,
